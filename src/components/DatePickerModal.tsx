...
export const DatePickerModal: React.FC<DatePickerModalProps> = ({
  ...
  const [currentMonth, setCurrentMonth] = useState<Date>(() => safeParseDate(initialDate));
  const [selectedDate, setSelectedDate] = useState<Date>(() => safeParseDate(initialDate));

  useEffect(() => {
    if (visible) {
      const parsed = safeParseDate(initialDate);
              ...
              <View className="flex-row items-center justify-between gap-3 pt-3 border-t border-white/10">
                <View className="flex-1">
                  ...
                  <Text className="text-emerald-400 font-mono font-bold text-sm mt-0.5">
                    {formatDateString(selectedDate) || "YYYY-MM-DD"}
                  </Text>
                </View>
